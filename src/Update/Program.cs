    ...
    class Program : IEnableLogger
    {
        ...
        int executeCommandLine(string[] args)
        {
            ...
            using (Disposable.Create(() => animatedGifWindowToken.Cancel()))
            {
                ...
                string prevReleaseUrl = default(string);
        ...
        public void Releasify(string package, string targetDir = null, string packagesDir = null, string bootstrapperExe = null, string backgroundGif = null, string signingOpts = null, string baseUrl = null, string setupIcon = null, bool generateMsi = true, string prevReleaseUrl = null)
        {
            ...
            if (!string.IsNullOrEmpty(prevReleaseUrl))
            {
                previousReleases.AddRange(ReleaseEntry.ParseReleaseFile(DownloadReleaseFile(prevReleaseUrl).Result));
            }
            else if (File.Exists(releaseFilePath))
            {
            ...
            foreach (var file in toProcess)
            {
                ...
                var prev = ReleaseEntry.GetPreviousRelease(previousReleases, rp, targetDir, prevReleaseUrl, this.Log());
